...
public class ExceptionTypes {
     ...
     * to be explicitly be thrown in method signature.
     * Or has to be caught {}
     * When may this type of exception be used?
     * When there is no need to handle the exception.
     ...
     * Is it a good practice to not propagate?
     * If there is a caller, then we have to propagate.
     * If there is a client/ user, then we do not have to propagate the
     * exception.
     * @return
     */
    ...
    public static char readAgainOneChar(char ch) throws IOException, IllegalArgumentException{
        try {
            ...
            if (inputChar != ch) throw new IllegalArgumentException("Char is not fit");
            return inputChar;
